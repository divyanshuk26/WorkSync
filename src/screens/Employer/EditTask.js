...
export default function EditTask({ route, navigation }) {
  ...
  const [selectedEmployee, setSelectedEmployee] = useState(
    task.assigned_profile || (task.assigned_to ? { id: task.assigned_to } : null)
  );
  const [priority, setPriority] = useState(formatPriority(task.priority));
  const [deadline, setDeadline] = useState(task.deadline || '');
  const [status, setStatus] = useState(task.status || TASK_STATUS.PENDING);
  ...
  useEffect(() => {
    const loadEmployees = async () => {
      try {
        ...
        setEmployees(data);
        if (task.assigned_to && !selectedEmployee?.full_name) {
          const found = data.find((e) => e.id === task.assigned_to);
          if (found) setSelectedEmployee(found);
        }
      } catch (err) {
        console.warn('Failed to load employees for task edit:', err);
      } finally {
  ...
  return (
    ...
    >
      <AppHeader title="Edit Task" subtitle="Modify task details, priority & assignee" />
              ...
              {[
                ...
                return (
                  <TouchableOpacity
                    ...
                    onPress={() => !isSubmitting && setStatus(st.value)}
                    activeOpacity={0.8}
                    disabled={isSubmitting}
                  >
                    <Text style={[styles.priorityText, isSelected && styles.priorityTextActive]}>
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  ...
                  >
                    ...
                    <Text style={styles.employeeItemSub}>
                      {item.email} {item.department ? `• ${item.department}` : ''}
                    </Text>
                  </TouchableOpacity>
                )}
              />
